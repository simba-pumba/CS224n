...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0 :
        next_trans = model.predict(unfinished_parses[:batch_size])
        for curr_trans, curr_parse in zip(next_trans, unfinished_parses[:batch_size]):
            curr_parse.parse_step(curr_trans)
            if (len(curr_parse.buffer) == 0) and (len(curr_parse.stack) == 1):
                unfinished_parses.remove(curr_parse)
